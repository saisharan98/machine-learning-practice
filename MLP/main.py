...
# print(np.std(train_x, axis=0).shape)
...
plot2d(hid_features,np.argmax(train_y,axis=-1),"Hid_Features vs np.argmax(train_y,axis=-1)",'train')
# validation set visualization
hid_features = clf.get_hidden(valid_x)
plot2d(hid_features,valid_y,"Hid_Features vs valid_y",'valid')
# test set visualization
hid_features = clf.get_hidden(test_x)
plot2d(hid_features,test_y,"Hid_Features vs test_y",'test')
...
# training set visualization
hid_features = clf.get_hidden(train_x)
plot3d(hid_features,np.argmax(train_y,axis=-1),"3D Hid_Features vs np.argmax(train_y,axis=-1)",'train')
# validation set visualization
hid_features = clf.get_hidden(valid_x)
plot3d(hid_features,valid_y,"3D Hid_Features vs valid_y",'valid')
# test set visualization
hid_features = clf.get_hidden(test_x)
plot3d(hid_features,test_y,"3D Hid_Features vs test_y",'test')
